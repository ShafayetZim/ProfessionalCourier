        ...
        if request.method == "POST":
            ...
            if(email != '' and username != '' and password != '' and password1 != '' ):
                if User.objects.filter(username=username).exists():
                    ...
                    if form.is_valid():
                        subject = "Welcome to Tpc  Membership"
...
def register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, f'Account has been created for {username}! S/He Can login now.')
            return redirect('dashboard')
    else:
        form = UserRegisterForm()
    return render(request, 'pages/authentication/register.html', {'form': form,
                                                                  'title': "Register", 'pageview': "Authentication"
                                                                  })
...
def profile(request):
    ...
    return render(request, 'pages/authentication/user-profile.html', context)